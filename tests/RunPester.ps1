...
Set-Location "C:\OPEN_PROJECTS\PowerCLIExamples\"
...
foreach ($Directory in $Directories)
{
    ...
    $files = Get-ChildItem .\PowerCLIExamples\$Directory
    foreach ($file in $files)
    {
        # Source the file
        $FileName = $file.Name
        Write-Output "Importing file $FileName"
        . .\PowerCLIExamples\$Directory\$FileName
    }
    # Execute Pester for the Directory.
    $results = Invoke-pester .\tests\$Directory\*.ps1 -CodeCoverage .\PowerCLIExamples\$Directory\*.ps1 -PassThru -ExcludeTag SkipThis
